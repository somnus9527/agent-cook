...
export function createApp(opts: AppOptions) {
  ...
  const deps: Deps = {
    ...
    emit: makeEmit(sinks),
    store,
    toolSchemas: (opts.tools ?? []).map((t) => t.schema),
  };
  ...
  return {
    async run(): Promise<void> {
      ...
      try {
        // ── resume 启动路径（16 §6）──
        let state =
          (opts.resumeSessionId ? await store.load(opts.resumeSessionId) : null) ??
          initState(config);
        ...
        for (;;) {
          ...
          // 选编排范式并跑（loop 内部每 step 已自行 store.save）。
          const runner = manager.select(state.mode);
...
function initState(config: AgentConfig): AgentState {
  return {
