...
├─ AgentLoopManager   选 ReAct / Plan-Execute 策略，调 loop.run()   │
...
**入门建议**：先只做 `AgentReActLoop`，把规划做成一个工具试水（见 §5）；`AgentLoopManager` 的 Strategy 口子先留着（一个 `mode` 配置 + 一个 `selectLoop()`），等真有复杂长任务再补 `PlanExecuteLoop`。别一上来写两套引擎。
